...
const DEFAULT_CONFIG = {
  ...
  releaseHiddenAfterMin: 5  // 窗口隐藏超过 N 分钟后释放 webview 页面内存（0 = 关闭该行为）
};
...
function applySpeedToAll() {
  ...
  const js = `(function(r){
    window.__pipSpeed = r;
    // 快速路径：video/audio 走标签索引，不遍历全树，代价极低
    function mediaIn(root) {
      var out = [];
      if (!root) return out;
      try {
        var m = root.querySelectorAll('video,audio');
        for (var i = 0; i < m.length; i++) out.push(m[i]);
      } catch(e) {}
      return out;
    }
    // 深度路径：翻 Shadow DOM。querySelectorAll('*') 是全树遍历，代价高，
    // 所以只在快速路径一无所获时才走，并且额外做 3 秒节流——
    // 原实现对每次调用都做全量遍历 + 递归，被 MutationObserver 高频触发时是持续吃 CPU/内存的主因。
    var lastDeepScan = 0;
    function deepMedia(root) {
      var out = mediaIn(root);
      try {
        var all = root.querySelectorAll('*');
        for (var j = 0; j < all.length; j++) {
          if (all[j].shadowRoot) out = out.concat(deepMedia(all[j].shadowRoot));
        }
      } catch(e) {}
      return out;
    }
    function findMedia(root) {
      var fast = mediaIn(root);
      if (fast.length) return fast;
      var now = Date.now();
      if (now - lastDeepScan < 3000) return fast;
      lastDeepScan = now;
      return deepMedia(root);
    }
    ...
    if (!window.__pipSpeedObs) {
      // 防抖：页面 DOM 变动非常频繁，攒 300ms 统一处理一次。
      // 原实现是每次变动直接 setSpeed()，等于把上面的全量扫描频率拉满。
      var pend = false;
      window.__pipSpeedObs = new MutationObserver(function(){
        if (pend) return;
        pend = true;
        setTimeout(function(){ pend = false; setSpeed(); }, 300);
      });
...
// 请求拦截（域名黑名单）+ UA 伪装。
// 说明：这里只做 AD_HOSTS 域名级拦截，未接入 EasyList 规则表。
// 原因是 EasyList 的元素隐藏规则会误伤视频播放器覆盖层（播放按钮等），导致点击无效。
...
// ===== 隐藏超时后释放 webview 页面内存 =====
// 老板键把窗口藏起来时，webview 里的网页（DOM / JS 堆 / 已解码图片）仍然整份占着内存，
// 实测单个网页渲染进程独占内存能到 290MB 以上。隐藏超过 N 分钟后把它导航到 about:blank，
// 页面内存随即释放；窗口重新显示时自动加载回原地址（session/登录态不受影响）。
// N 来自 config.releaseHiddenAfterMin，设 0 可关闭该行为。
let webviewReleaseTimer = null;
let releasedWebviewUrl = '';

function releaseHiddenAfterMs() {
  const min = config.releaseHiddenAfterMin === undefined ? 5 : Number(config.releaseHiddenAfterMin) || 0;
  return min > 0 ? min * 60 * 1000 : 0;
}

function disarmWebviewRelease() {
  if (webviewReleaseTimer) { clearTimeout(webviewReleaseTimer); webviewReleaseTimer = null; }
}

function armWebviewRelease() {
  disarmWebviewRelease();
  const ms = releaseHiddenAfterMs();
  if (!ms) return;
  webviewReleaseTimer = setTimeout(function () {
    webviewReleaseTimer = null;
    if (!hidden || !mainWindow || mainWindow.isDestroyed()) return;
    const wc = getWebviewContents();
    if (!wc) return;
    let cur = '';
    try { cur = wc.getURL() || ''; } catch (_) {}
    // 没有真实网页（空、about:、本地欢迎页）就没什么可释放的，别动
    if (!cur || cur.indexOf('about:') === 0 || cur.indexOf('file:') === 0) return;
    releasedWebviewUrl = cur;
    console.log('[mem] 窗口已隐藏超过 ' + (releaseHiddenAfterMs() / 60000) + ' 分钟，释放网页内存:', cur);
    try { wc.loadURL('about:blank'); } catch (_) {}
  }, ms);
}

function restoreWebviewIfReleased() {
  if (!releasedWebviewUrl) return;
  const url = releasedWebviewUrl;
  releasedWebviewUrl = '';
  const wc = getWebviewContents();
  if (wc) {
    console.log('[mem] 窗口重新显示，恢复网页:', url);
...
function setHidden(next) {
  ...
  if (hidden) {
    ...
    // 如果隐藏期间释放过网页内存，这里加载回来
    restoreWebviewIfReleased();
  }
...
// ===== 外部窗口透明度用的常驻 PowerShell 进程 =====
// 这个进程是常驻的（脚本里 while($true){ReadLine}），实测占用约 76MB，
// 外加配套 conhost 约 8MB。原实现只在 ensureWinOpPs 内部持句柄、退出时也不回收，
// 实测会留下孤儿 powershell.exe 永久常驻（已复现：其 ParentPID = PipReader 主进程）。
// 现在改为：空闲 90 秒自动回收 + app 退出强制回收，下次用到时按需拉起（约 200-500ms 延迟）。
let winOpPsRef = null;
let winOpIdleTimer = null;
const WINOP_IDLE_MS = 90 * 1000;

function killWinOpPs() {
  if (winOpIdleTimer) { clearTimeout(winOpIdleTimer); winOpIdleTimer = null; }
  const p = winOpPsRef;
  winOpPsRef = null;
  if (p) {
...
// 按需查找 webview 的 webContents。释放内存时必须实时查，不能依赖早先缓存的引用
// （缓存的 webviewContents 只在创建时赋值一次，可能指向已失效或非 webview 的实例）。
function getWebviewContents() {
  const { webContents } = require('electron');
  const all = webContents.getAllWebContents();
  for (let i = 0; i < all.length; i++) {
    try {
      if (!all[i].isDestroyed() && all[i].getType() === 'webview') return all[i];
    } catch (_) {}
  }
  return null;
}
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      webviewTag: true,
      // 中文场景用不到拼写检查，关掉可免去 Chromium 为每个输入框常驻的拼写检查服务
      spellcheck: false
    }
  });
...
function wireIpc() {
  ...
  function ensureWinOpPs() {
    if (winOpPs && !winOpPs.killed && winOpPs.exitCode === null) return winOpPs;
    winOpPs = null;
    const script = [
      'Add-Type @"',
    ...
    const forget = () => {
      if (winOpPs === proc) winOpPs = null;
      if (winOpPsRef === proc) winOpPsRef = null;
    };
    proc.on('close', forget);
    proc.on('error', forget);
    winOpPs = proc;
    winOpPsRef = proc;   // 模块级引用，供空闲回收 / 退出回收使用
    return proc;
  }
  ...
  ipcMain.handle('set-win-opacity', (_e, hwnd, opacity) => {
    ...
    if (!ps) return false;
    // 用完开始计时：空闲 90 秒就回收这个约 76MB 的常驻 PowerShell，下次用到再拉起
    if (winOpIdleTimer) clearTimeout(winOpIdleTimer);
    winOpIdleTimer = setTimeout(killWinOpPs, WINOP_IDLE_MS);
...
if (!gotLock) {
  ...
  app.on('will-quit', () => {
    globalShortcut.unregisterAll();
    killWinOpPs();            // 回收常驻 PowerShell（约 76MB），否则会留下孤儿进程
    disarmWebviewRelease();
    flushConfig();
    flushProgress();
  });
